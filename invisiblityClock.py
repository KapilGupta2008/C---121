...
#aallowing the webcam to start by making the code slep for 2 seconds
time.sleep(2)
bg=0
...
while(cap.isOpened()):
    ...
    #convertingthe color from BGR to HSV
    ...
    parameters:
    ● src − An object representing
    the source (input) image.
    ● dst − object representing the
    destination (output) image.
    op − An integer representing
    the type of the Morphological
    operation.
    ● kernel − A kernel matrix morphologyEx() is the method of the
    class Img Processing which is used to
